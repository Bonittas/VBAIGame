...
import threading
  ...
  # Use native macOS window system
...
# Set the display mode (initialize the video system)
display = (800, 600)
pygame.display.set_mode(display, DOUBLEBUF | OPENGL)

# Set OpenGL context version after initializing the display
...
class DialogueSystem:
    ...
    def get_npc_response(self, npc_role, user_input):
        print(f"[DEBUG] Generating response for {npc_role}...")

        # Use predefined responses if available
        responses = {
            "hello": "Hello! How can I help you today?",
            "who are you": f"I am the {npc_role} of this company.",
            "what is your job": f"My role as {npc_role} is to ensure the company's success.",
        ...
        # Otherwise, generate a dynamic AI response using GPT
        try:
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": f"You are a {npc_role} in a virtual world."},
                    {"role": "user", "content": user_input}
                ]
            )
...
client = openai.OpenAI()  # Ensure you've initialized the OpenAI client

# Record audio and save it as a WAV file


# Initialize OpenAI client (Make sure your API key is set correctly)
@sio.event
async def connect(sid, environ):
    print(f"[INFO] Client connected: {sid}")  # ✅ Debug log to confirm connection
    clients[sid] = {}
    await sio.emit("server_message", "Connected successfully!", to=sid)
...
async def audio_data(sid, data):
    """Receive raw audio data from client and process it."""
    print(f"[INFO] Received audio from {sid}")
    filename = f"audio_{sid}.wav"
    ...
    # Generate speech
    speech_audio = generate_speech(ai_response)
    if speech_audio:
        await sio.emit("audio_response", speech_audio, to=sid)
    
# Function to transcribe audio


def record_audio(filename="output.wav", duration=3, samplerate=16000):
    ...
    return filename  # Return recorded file path
...
def play_audio(audio_content):
    ...
    if not audio_content:
        print("[ERROR] No audio response available.")
        return

    temp_filename = "response.wav"

    # Save AI-generated speech as a temporary file
    with open(temp_filename, "wb") as f:
        f.write(audio_content)

    # Read and play the audio
    data, samplerate = sf.read(temp_filename, dtype="int16")

    time.sleep(0.1)  # ✅ Add a delay to prevent playback crashes
...
def get_ai_response(text):
    try:
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an AI assistant in a 3D adventure game."},
...
def handle_speech(dialogue_system, npc_role):
    with speech_lock:  # Ensure only one thread runs at a time
        print(f"[INFO] Handling speech with {npc_role}...")
        ...
        if not transcription:
            print("[ERROR] No valid transcription.")
            return 

        print(f"[DEBUG] User said: {transcription}")

        # Step 2: Send text to AI
        npc_response = dialogue_system.send_text_to_ai(transcription)
        if not npc_response:
            print("[WARNING] No response generated.")
            return 

        print(f"[DEBUG] AI Responded: {npc_response}")

        # Step 3: Convert AI response to voice 
        speech_audio = generate_speech(npc_response)
        if speech_audio:
            play_audio(speech_audio)  # ✅ Prevent overlapping playback
        else:
...
class DialogueSystem:
    def __init__(self):
        ...
        self.last_input_text = ""   # Track input text separately
        self.conversation_history = []  # Maintain conversation history
        self.waiting_for_response = False # Flag to prevent sending multiple requests
        self.npc_role_talking_to = None  # Track which NPC we're talking to (HR or CEO)
        # Create a surface for the UI
        self.ui_surface = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SRCALPHA).convert_alpha()
        self.ui_texture = glGenTextures(1)
        self.current_npc = None  # Track which NPC we're talking to
    ...
    def start_conversation(self, npc_role="HR", player_pos=None):
        ...
        conversation_active = True  # Set conversation flag
        self.active = True
        self.input_active = True
        self.current_npc = npc_role
        self.npc_role_talking_to = npc_role # Track the role
        # Store player's position when starting conversation
        self.initial_player_pos = [player_pos[0], player_pos[1], player_pos[2]] if player_pos else [0, 0.5, 0]
        print(f"[DialogueSystem] Dialogue started with {npc_role}")
    ...
    def send_message(self):
        global conversation_active  # Access the global flag
        if not self.conversation_history:
            print("[DialogueSystem] No conversation history to send.")
            return

        if self.waiting_for_response:
            print("[DialogueSystem] Already waiting for a response, ignoring.")
            return
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if keys[pygame.K_LSHIFT] and event.key == pygame.K_q:
                self.active = False
                self.input_active = False
                conversation_active = False  # Clear conversation flag
                print("[DialogueSystem] Chat ended")
                # Return both the command and the initial position
                return {"command": "move_player_back", "position": self.initial_player_pos}
            # Check for Shift + C to stop conversation, but persist it.
            elif keys[pygame.K_LSHIFT] and event.key == pygame.K_c:
                self.active = False  # Stop displaying the dialogue box
                self.input_active = False # Deactivate input
                print("[DialogueSystem] Conversation paused.")
                return # Exit handler without any command.
    ...
    def continue_conversation(self):
        """Continues a previously paused conversation."""
        global conversation_active
        if not self.active and self.conversation_history: # Only continue when not already active and there is a conversation
            self.active = True
            self.input_active = True
            conversation_active = True
            print("[DialogueSystem] Conversation resumed.")
...
class Game3D:
    def __init__(self):
        ...
        self.conversation_active = False  # Initialize conversation_active
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_ESCAPE:
                            ...
                            npc_role = "HR" if hr_distance < ceo_distance else "CEO"
 
                    
                            speech_thread = threading.Thread(target=handle_speech, args=(self.dialogue, npc_role))

                            speech_thread.daemon = True  # Prevents blocking the main game loop
                            speech_thread.start()
